...
module.exports = {
    ...
    obterFipe : async ()=>{
        try {
            let res = await axios.get(`https://api.invertexto.com/v1/fipe/brands/1?token=${process.env.API_FIPE_CARRO.trim()}`), resposta = []
            for(var fipe of res.data.articles){
                resposta.push({
                    titulo : fipe.title,
                    descricao : fipe.description,
                    url : fipe.url
                })
            }
            return resposta
        } catch(err){
            consoleErro(msgs_texto.api.newsapi, "API obterFipes")
            throw new Error(msgs_texto.utilidades.fipe.indisponivel)
        }
    },
    ...
    consultarCEP: async (cep) => {
        try {
            ...
            // Faz a requisição para o serviço de consulta de CEP
    ...
    consultarCNPJ: async (cnpj) => {
        try {
            ...
            // Verifica se o CNPJ possui o formato correto
            if (!/^\d{14}$/.test(cnpj)) {
                throw new Error("CNPJ inválido, digite apenas os números.");
            }
            
            // Faz a requisição para o serviço de consulta de CNPJ
    ...
    consultarFeriados: async (ano, estado) => {
        try {
            // Faz a requisição para o serviço de consulta de feriados
            const response = await fetch(`https://api.invertexto.com/v1/holidays/${ano}?token=7085|MVzVV8MJsYiqEteTT0WAdGrPAylCvDTj`);
            
            // Verifica se a resposta da requisição foi bem-sucedida
            if (!response.ok) {
                throw new Error("Erro ao consultar os feriados");
            }
            
            // Extrai os dados da resposta JSON
            const feriados = await response.json();
            
            // Formata as datas dos feriados para o padrão brasileiro
            feriados.forEach(feriado => {
                const data = new Date(feriado.date);
                feriado.date = data.toLocaleDateString('pt-BR');
            });
            
            // Retorna os feriados do ano especificado
            return feriados;
        } catch (error) {
            // Manipula os erros
            console.error(error.message);
